...
async def fetch_products(domain, proxy_str=None):
    try:
        ...
        return False, f"PROXY_ERROR: {str(e)[:50]}"
    except Exception as e:
...
def extract_clean_response(message):
    ...
    # Error code mapping - convert GENERIC_ERROR to proper codes
    ERROR_MAPPING = {
        'GENERIC_ERROR': 'CARD_DECLINED',
        'MISMATCHED_BILL': 'CARD_DECLINED',
        'Unknown Result': 'CARD_DECLINED',
        'Unknown reason': 'CARD_DECLINED',
        'No receipt in submit response': 'CARD_DECLINED',
        'No receipt ID': 'CARD_DECLINED',
        'Submit Rejected': 'CARD_DECLINED',
        'Throttled': 'SITE_BUSY',
        'Checkpoint Denied': 'SITE_BLOCKED',
        'Negotiation failed': 'SITE_ERROR',
        'Session is null': 'SITE_ERROR',
        'Result is null': 'SITE_ERROR',
        'No delivery data in proposal': 'SITE_ERROR',
        'No runningTotal in sellerProposal': 'SITE_ERROR',
        'No valid payment method found': 'SITE_ERROR',
    ...
    for pattern in patterns:
        ...
        for match in matches:
            ...
            if match and "_" in match and len(match) < 50:
                match = match.strip("{}:'\" ")
                # Check mapped errors
                if match in ERROR_MAPPING:
                    return ERROR_MAPPING[match]
                if 'GENERIC_ERROR' in match:
                    return 'CARD_DECLINED'
                return match
    ...
    if words:
        ...
        if "_" in first_word and first_word.isupper():
            if first_word in ERROR_MAPPING:
                return ERROR_MAPPING[first_word]
            return first_word
    
    # If nothing matched, return CARD_DECLINED for unknown errors
    return 'CARD_DECLINED'
...
async def process_card(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None):
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            if cart_resp.status != 200:
                return False, f"CART_FAILED_{cart_resp.status}", gateway, total_price, currency
            ...
            if 'login' in checkout_url.lower():
                return False, "SITE_REQUIRES_LOGIN", gateway, total_price, currency
            ...
            # Detect gateway from checkout page
            if 'Shopify Payments' in text or 'shopify_payments' in text:
                gateway = 'Shopify Payments'
            elif 'Stripe' in text or 'stripe' in text:
                gateway = 'Stripe'
            elif 'Braintree' in text or 'braintree' in text:
                gateway = 'Braintree'
            elif 'PayPal' in text or 'paypal' in text:
                gateway = 'PayPal'
            elif 'Adyen' in text or 'adyen' in text:
                gateway = 'Adyen'
            elif 'Checkout.com' in text or 'checkout.com' in text:
                gateway = 'Checkout.com'
            elif 'Square' in text or 'square' in text:
                gateway = 'Square'
            elif 'Authorize.net' in text or 'authorizenet' in text:
            ...
            if not sst:
                return False, "SESSION_TOKEN_FAILED", gateway, total_price, currency
            ...
            try:
                ...
                if not token:
                    return False, 'PAYMENT_TOKEN_FAILED', gateway, total_price, currency
            except Exception as e:
            ...
            if 'CAPTCHA_REQUIRED' in final_text:
                return True, "CAPTCHA_REQUIRED", gateway, total_price, currency
            
            if 'WaitingReceipt' in final_text:
                return False, "SITE_BUSY", gateway, total_price, currency
