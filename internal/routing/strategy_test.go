...
func TestRoundRobinSelector_Select(t *testing.T) {
	...
	// Reset by creating new selector for each test
	routes := []domain.Route{
	...
	// With 2 routes, first and third should be the same (wrap around)
...
func TestWeightedSelector_Select(t *testing.T) {
	// High-weight route should be selected more often.
	routes := []domain.Route{
	...
	// Heavy should be selected more than light
	if counts["heavy"] <= counts["light"] {
		t.Errorf("expected heavy (weight=90) > light (weight=10), got heavy=%d light=%d",
...
func TestWeightedSelector_Select_ZeroWeights(t *testing.T) {
	s := NewWeightedSelector()
	routes := []domain.Route{
		route("first", "1", "conn-1", 10, true),
		route("second", "1", "conn-2", 10, true),
	}
	// Zero weight — should fall back to first
	selected, err := s.Select(routes)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
